...
export const fetchCoffeeRecipes = async (prompt: string) => {
  try {
    const ai = getAiInstance();
...
export const fetchCountryList = async (prompt: string) => {
    try {
        const ai = getAiInstance();
        const response = await ai.models.generateContent({
            model: "gemini-2.5-flash",
