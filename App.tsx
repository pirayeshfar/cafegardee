...
import { CoffeeBeanIcon, WorldIcon, ArrowRightIcon, TeaIcon, ColdDrinkIcon, ChatIcon } from './components/icons.tsx';
import CountryCard from './components/CountryCard.tsx';
import RecipeCard from './components/RecipeCard.tsx';
import RecipeModal from './components/RecipeModal.tsx';
import Chatbot from './components/Chatbot.tsx';
import CategorySelectionCard from './components/CategorySelectionCard.tsx';
...
const App = () => {
  const [viewState, setViewState] = useState('initial'); // 'initial', 'subCategorySelection', 'countrySelection', 'recipeList'
  const [selectedType, setSelectedType] = useState<'قهوه' | 'دمنوش' | 'نوشیدنی' | null>(null);
  ...
  const handleTypeSelect = (type: 'قهوه' | 'دمنوش' | 'نوشیدنی') => {
  ...
  return (
    <div className="bg-[#DCD7C9] min-h-screen text-[#4A2C2A]">
      <style>{`
      ...
      {isApiKeyMissing && (
          <div className="bg-yellow-200 border-b-2 border-yellow-400 text-yellow-800 p-3 text-center text-sm font-semibold" role="alert">
              <strong>هشدار:</strong> کلید API برای هوش مصنوعی تنظیم نشده است. بخش چت‌بات کار نخواهد کرد. لطفاً متغیر محیطی `API_KEY` را در هاست خود تنظیم کنید.
          </div>
      )}

      <header className="py-6 px-4 text-center bg-[#4A2C2A]/10 backdrop-blur-sm flex flex-col items-center">
        <div className="w-20 h-20 bg-[#4A2C2A] rounded-full flex items-center justify-center mb-4 shadow-lg animate-fade-in"><CoffeeBeanIcon className="w-12 h-12 text-[#DCD7C9]" /></div>
        <h1 className="text-4xl md:text-5xl font-bold text-[#4A2C2A]">کافه گردی</h1>
        <p className="mt-2 text-lg text-[#A27B5C]">راهنمای جامع نوشیدنی‌های کافه</p>
      </header>

      <main className="container mx-auto p-4 md:p-8">
        {viewState !== 'initial' && (
            <div className="text-center mb-8 animate-fade-in">
                <button onClick={handleGoBack} className="group relative inline-flex items-center justify-center px-6 py-3 font-bold text-[#4A2C2A] transition-all duration-300 ease-in-out bg-white/60 rounded-lg shadow-md hover:bg-white overflow-hidden">
                    <span className="absolute top-0 right-0 w-0 h-full transition-all duration-300 ease-in-out bg-[#A27B5C] group-hover:w-full"></span>
                    <span className="relative z-10 flex items-center gap-2 transition-colors duration-300 group-hover:text-white"><ArrowRightIcon className="w-5 h-5 transition-transform duration-300 group-hover:translate-x-1" />{getBackButtonText()}</span>
                </button>
            </div>
        )}
        ...
        {viewState === 'initial' && (
          <div className="animate-fade-in-up">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-center">به کافه گردی خوش آمدید!</h2>
            <p className="text-lg md:text-xl mb-12 max-w-3xl mx-auto text-center">دنیای شگفت‌انگیز نوشیدنی‌ها را کشف کنید. برای شروع، یکی از دسته‌بندی‌های زیر را انتخاب کنید.</p>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <CategorySelectionCard title="دنیای قهوه" description="از اسپرسو تا کلد برو" imageUrl="https://source.unsplash.com/800x600/?coffee,dark" icon={<CoffeeBeanIcon className="w-12 h-12" />} onSelect={() => handleTypeSelect('قهوه')} />
              <CategorySelectionCard title="باغ دمنوش‌ها" description="آرامش با طعم طبیعت" imageUrl="https://source.unsplash.com/800x600/?tea,leaves" icon={<TeaIcon className="w-12 h-12" />} onSelect={() => handleTypeSelect('دمنوش')} />
              <CategorySelectionCard title="نوشیدنی‌های خنک" description="خنکای لذت‌بخش" imageUrl="https://source.unsplash.com/800x600/?cocktail,juice" icon={<ColdDrinkIcon className="w-12 h-12" />} onSelect={() => handleTypeSelect('نوشیدنی')} />
            </div>
          </div>
        )}

        {viewState === 'subCategorySelection' && selectedType && (
            <div className="text-center py-10 animate-fade-in-up">
                <h2 className="text-3xl md:text-4xl font-bold mb-4">{typeTitles[selectedType]}</h2>
                <p className="text-lg md:text-xl mb-8 max-w-3xl mx-auto">روش کاوش خود را انتخاب کنید.</p>
                <div className="flex flex-col md:flex-row gap-4 justify-center max-w-4xl mx-auto">
                    <button onClick={handleShowClassic} className="flex-1 flex items-center justify-center gap-3 bg-[#4A2C2A] text-white text-lg font-semibold py-4 px-8 rounded-xl shadow-lg hover:bg-[#382220] transform hover:-translate-y-1 transition-all duration-300"><CoffeeBeanIcon className="w-6 h-6" />مرجع رسپی‌های کلاسیک</button>
                    <button onClick={handleShowCountries} className="flex-1 flex items-center justify-center gap-3 bg-[#A27B5C] text-white text-lg font-semibold py-4 px-8 rounded-xl shadow-lg hover:bg-[#8e694d] transform hover:-translate-y-1 transition-all duration-300"><WorldIcon className="w-6 h-6" />رسپی بر اساس کشورها</button>
                </div>
            </div>
        )}
        
        {viewState === 'countrySelection' && countries.length > 0 && (
            <div className="animate-fade-in">
                <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">یک کشور را انتخاب کنید</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">{countries.map((c, i) => <CountryCard key={c} countryName={c} index={i} onSelect={() => handleSelectCountry(c)} />)}</div>
            </div>
        )}
        
        {viewState === 'recipeList' && currentRecipes.length > 0 && (
          <div className="animate-fade-in">
            <h2 className="text-2xl md:text-3xl font-bold text-center mb-8">{listTitle}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{currentRecipes.map((r, i) => <RecipeCard key={`${r.name}-${i}`} recipe={r} index={i} onView={() => setSelectedRecipe(r)} />)}</div>
          </div>
        )}
      </main>
      ...
      <button 
        ...
        className="fixed bottom-6 right-6 bg-[#4A2C2A] text-white w-16 h-16 rounded-full shadow-lg flex items-center justify-center transform hover:scale-110 transition-transform duration-300 z-40"
        aria-label="باز کردن چت با باریستای هوشمند"
      >
      ...
      <footer className="text-center py-6 mt-12 border-t border-[#A27B5C]/30">
        <p className="text-[#A27B5C]">طراحی شده با عشق برای علاقه‌مندان به نوشیدنی &copy; {new Date().getFullYear()}</p>
      </footer>
    </div>
  );
};
